...
```sh
docker compose exec api bash -c 'php artisan migrate:fresh && su -s /bin/sh www-data -c "php artisan passport:install --no-interaction" && php artisan db:seed && php artisan key:generate && php artisan storage:link'
```
...
docker compose exec --user www-data api php artisan passport:install --no-interaction
